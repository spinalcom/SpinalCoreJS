...
class root.SpinalUserManager
  ...
  @change_password_by_admin: (options, username, password, admin_id, admin_password, success_callback, error_callback = null)->
    # Access: /get_change_user_password_by_admin?u=<username>&np=<newpass>&a=<admin_id>&ap=<adminPass>
  ...
  @change_account_rights_by_admin: (options, username, right, admin_id, admin_password, success_callback, error_callback = null)->
    # Access: /get_change_user_password_by_admin?u=<username>&ri=<rights>&a=<admin_id>&ap=<adminPass>
    # admin == 644(root) or 168(admin)
